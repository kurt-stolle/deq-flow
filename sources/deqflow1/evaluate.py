...
def create_sintel_submission(
    model, warm_start=False, fixed_point_reuse=False, output_path="sintel_submission", **kwargs
):
...
def validate_things(model, **kwargs):
    ...
    for dstype in ["frames_cleanpass", "frames_finalpass"]:
        ...
        print(
            "Validation w/ mask (%s) EPE: %f, 1px: %f, 3px: %f, 5px: %f"
...
def validate_sintel(model, **kwargs):
    ...
    for dstype in ["clean", "final"]:
        ...
        print(
            f"Validation ({dstype}) EPE: {epe} ({best[dstype+'-epe'] if best[dstype+'-epe'] < 1e8 else 'N/A'}), 1px: {px1}, 3px: {px3}, 5px: {px5}"
        )
        results[dstype] = np.mean(epe_list)
...
def validate_kitti(model, **kwargs):
    ...
    print(
        f"Validation KITTI: EPE: {epe} ({best['epe'] if best['epe'] < 1e8 else 'N/A'}), F1: {f1} ({best['f1'] if best['f1'] < 1e8 else 'N/A'})"
    )
