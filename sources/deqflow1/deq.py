...
class DEQFlowBase(nn.Module):
    ...
    def _log_convergence(self, result, name="FORWARD", color="yellow"):
        ...
        print(
            colored(
                f"{'TRAIN' if self.training else 'VALID'} | {name} {stop_mode}_diff: {stop_diff}; {alt_mode}_diff: {alt_diff}; nstep: {result['nstep']}",
                f"{color}",
            )
        )
    ...
    def _fixed_point_solve(self, deq_func, z_star, seed=None, f_thres=None, **kwargs):
    ...
    def forward(
        ...
        **kwargs,
        ...
        return self._deq_forward(
            deq_func, list2vec, vec2list, z_star, coords0, flow_gt, valid, step_seq_loss, seed, sradius_mode, **kwargs
        )
        ...
        **kwargs,
        ...
        if self.training:
            ...
            z_star, _, rel_error, abs_error = self._fixed_point_solve(
                deq_func, z_star, seed=seed, f_thres=self.eval_f_thres
            )
    ...
    def _deq_forward(
        ...
        **kwargs,
        ...
        if self.training:
            ...
            # During inference, we directly solve for fixed point
            z_star, rel_error, abs_error = self._fixed_point_solve(
